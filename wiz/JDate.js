...
var Lunisolar = (function(global){
    global.JDate = /** @class */ (function () {
        function JDate(jdn) {
            this.jdn = jdn || 0;
        }
        //输入公历年，返回秒Delta Time
        JDate.dt = function (year) {
        ...
        JDate.DD = function (jd) {
            var r = {};
            var D = Math.floor(jd + 0.5), F = jd + 0.5 - D, c;
            if (D >= 2299161)
                c = Math.floor((D - 1867216.25) / 36524.25), D += 1 + c - Math.floor(c / 4);
            D += 1524;
        ...
        JDate.JD2str = function (jd) {
            var r = this.DD(jd);
            return this.DD2str(r);
        };
        JDate.timeStr = function (jde) {
            var h, m, s;
            jde += 0.5;
