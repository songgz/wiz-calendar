var EPHEM = (function () {
    ...
    eph.earth = (function () {
            return {
                lon: function (t) { //地球经度计算,返回Date分点黄经,传入世纪数、取项数
    ...
    eph.sun = (function(){
            return {
                ...
                aLon_t: function(W){ //已知太阳视黄经反求时间
                    var t,v = 628.3319653318;
                    t  = ( W - 1.75347 - Math.PI   ) / v;
                    v = eph.earth.v(t); //v的精度0.03%，详见原文
                    t += ( W - eph.sun.aLon(t) ) / v;
                    v = eph.earth.v(t); //再算一次v有助于提高精度,不算也可以
                    t += ( W - eph.sun.aLon(t) ) / v;
                    return t;
                },
        ...
        eph.ms = (function(){
            ...
            return {
                aLon:function(t){ //月日视黄经的差值
                    return eph.moon.lon(t) + eph.moon.gxcLon(t) - ( eph.earth.lon(t) + eph.sun.gxcLon(t) + Math.PI );
                },
                aLon_t:function(W){ //已知月日视黄经差求时间
                    var t, v = 7771.37714500204;
                    t  = ( W + 1.08472 )/v;
                    t += ( W - eph.ms.aLon(t) ) / v;
                    v = eph.moon.v(t) - eph.earth.v(t);  //v的精度0.5%，详见原文
                    t += ( W - eph.ms.aLon(t) ) / v;
                    t += ( W - eph.ms.aLon(t) ) / v;
                    return t;
                },
                aLon_t2:function(W){ //已知月日视黄经差求时间,高速低精度,误差不超过600秒(只验算了几千年)
                    ...
                    L = eph.moon.lon(t) - (4.8950632+ 628.3319653318*t + 0.000005297*t*t + 0.0334166*Math.cos(4.669257+628.307585*t) + 0.0002061*Math.cos(2.67823+628.307585*t)*t + 0.000349*Math.cos(4.6261+1256.61517*t) - 20.5/rad);
