var Lunar1 = (function () {
    ...
    lun.dn = function (u, r) { //计算农历节日 holiday
        //按农历日期查找重量点节假日
    ...
    lun.substr2 = function (s, n, end) { //截串(网页设计对过长的文字做截处理)
        s = s.replace(/(^\s*)|(\s*$)/g, "");
        if (s.length > n + 1) {
            s = s.substr(0, n) + end
        }
    ...
    lun.yueLiCalc = function (By, Bm) {
        ...
        for (i = 0, j = 0; i < Bdn; i++) {
            var ob = {
                d0: Bd0 + i, //儒略日,北京时12:00
                di: i,     //公历月内日序数
                y: By,    //公历年
                m: Bm,    //公历月
                dn: Bdn,   //公历月天数
                week0: opt.w0, //月首的星期
                week: (opt.w0 + i) % 7, //当前日的星期
                weeki: int2((opt.w0 + i) / 7), //本日所在的周序号
                weekN: int2((opt.w0 + Bdn - 1) / 7) + 1,  //本月的总周数
                yxmc: '', //月相名称
                yxjd: '', //月相时刻(儒略日)
                yxsj: '', //月相时间串
                jqmc: '', //定气名称
