...
var Ephem = __importStar(require("./Ephem"));
var int2 = function (v) { return Math.floor(v); };
var LunarDate = /** @class */ (function () {
    function LunarDate() {
        var options = [];
        ...
        switch (options.length) {
            case 0:
                ...
                break;
            case 1:
                this.jd = options[0] - JDate_1.JDate.J2000;
    ...
    LunarDate.prototype.yuerun = function (y1, m1) {
        ...
        if (Math.floor(hs + 0.5) > Math.floor(Ephem.sun.qi_accurate(w - LunarDate.PI2 / 12) + 0.5) && Math.floor(hs1 + 0.5) <= Math.floor(Ephem.sun.qi_accurate(w) + 0.5)) {
            for (j = 0; j <= 5; j++) {
                w += LunarDate.PI2 / 12;
                ms += LunarDate.PI2;
                if (Math.floor(Ephem.moon.so_accurate(ms) + 0.5) > Math.floor(Ephem.sun.qi_accurate(w) + 0.5))
                    return 0;
            }
            return 1;
        }
    ...
    LunarDate.prototype.ymdJd = function (y1, m1, rm, d1) {
        ...
        if (rm)
            rm = this.yuerun(y1, m1);
        if (rm == 0)
            return Math.floor(hs + d1 - 1 + 0.5) + JDate_1.JDate.J2000;
        else
            return Math.floor(hs1 + d1 - 1 + 0.5) + JDate_1.JDate.J2000;
    };
    LunarDate.prototype.jdYmd = function (jd) {
        var F, ms, jd1, jd2, w1, w2, wn, y, m, d, n, fd, ry, j;
        F = jd + 0.5 - Math.floor(jd + 0.5);
        jd = Math.floor(jd + 0.5) - JDate_1.JDate.J2000;
        ...
        while (Math.floor(Ephem.sun.qi_accurate(w1) + 0.5) < Math.floor(jd1 + 0.5))
            w1 += LunarDate.PI2 / 24;
        w2 = w1;
        while (Math.floor(Ephem.sun.qi_accurate(w2 + LunarDate.PI2 / 24) + 0.5) < Math.floor(jd2 + 0.5))
            w2 += LunarDate.PI2 / 24;
